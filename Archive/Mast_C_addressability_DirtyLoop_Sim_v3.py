...
B_Fields = np.arange(100,1800,150) #B_Field in Gauss
#Simulation Parameters
N_NV = 200#Number of NV Centres to generate
Gridsizes = [25, 35,50]
A_Max = 200e3 #Max Hyperfine interaction strength in Hz
A_Mins = [10e3,3e3,1e3] # Min Hyperfine interaction strength !Allowed to be lower at lower Conc
tau_max_ls = [10e-6,30e-6,100e-6] # max resonance time to look for in s !Allowed to be longer if lower Conc
tau_step = 1e-9 #timestep in s , was 2ns nu 1ns beste wat we experimenteel kunnen doen
max_gate_time_ls = [2e-3,6e-3,20e-3]# choose high because possible to correct in post proc
...
for idmu, Carbon_Conc in enumerate(Carbon_Concs):
    ...
    dir = 'simulation_data_werkbespreking'
